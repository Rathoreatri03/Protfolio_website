...
export function ContactTerminal() {
  ...
  return (
    <div className="relative group max-w-2xl w-full mx-auto">
      <form onSubmit={onSubmit} className="relative bg-[#0d0d0d] border border-white/10 rounded-lg shadow-2xl overflow-hidden font-mono text-[12px] group-hover:border-primary/20 transition-all duration-700">
        {/* Title Bar */}
        <div className="flex items-center justify-between px-4 py-2 bg-[#1a1a1a] border-b border-white/5">
          ...
          <div className="text-[9px] text-white/20 tracking-wider lowercase">{metadata.terminalUser}: ~/contact</div>
        ...
        <div className="p-6 space-y-6 relative">
          ...
          <div className="space-y-5">
            <TerminalField label="source-" highlight="NAME" comment="# [ENTER YOUR FULL NAME]" value={name} onChange={setName} placeholder="e.g. John Doe" terminalUser={metadata.terminalUser} />
            <TerminalField label="source-" highlight="EMAIL" comment="# [ENTER YOUR CONTACT EMAIL]" value={email} onChange={setEmail} type="email" placeholder="e.g. john@example.com" terminalUser={metadata.terminalUser} />
            
            <div className="space-y-2">
              <p className="text-[9px] text-primary/40 italic"># [TYPE YOUR MESSAGE BELOW]</p>
              <div className="flex items-center gap-2">
                <span className="text-primary/30 font-bold lowercase">{metadata.terminalUser}:~$</span>
              ...
              <div className="relative bg-white/[0.02] border border-white/5 rounded px-4 py-3 focus-within:border-primary/20 transition-all">
                <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} className="w-full bg-transparent border-none outline-none text-white font-medium placeholder:text-white/5 resize-none text-[13px]" placeholder="How can we collaborate? Describe your project here..." />
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-white/5 space-y-4">
            ...
            <button type="submit" disabled={sending} className="w-full py-4 bg-primary text-primary-foreground font-bold tracking-[0.2em] uppercase transition-all active:scale-[0.98] disabled:opacity-40 rounded flex items-center justify-center gap-3 group/btn">
              <span className="text-[11px]">{sending ? "TRANSMITTING..." : "sh ./SEND_MESSAGE.sh"}</span>
...
function TerminalField({ label, highlight, comment, value, onChange, type = "text", placeholder, terminalUser }: any) {
  return (
    <div className="space-y-2">
      <p className="text-[9px] text-primary/40 italic">{comment}</p>
      <div className="flex items-center gap-2">
        <span className="text-primary/30 font-bold lowercase">{terminalUser}:~$</span>
      ...
      <div className="relative bg-white/[0.02] border border-white/5 rounded px-4 h-10 flex items-center focus-within:border-primary/20 transition-all">
        <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-transparent border-none outline-none text-white font-bold placeholder:text-white/5 text-[13px]" autoComplete="off" placeholder={placeholder} />
      </div>
    </div>
  );
}
