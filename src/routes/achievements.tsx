...
function Achievements() {
  ...
  const activeCardIndex = useActiveOnScroll(".achievement-card", isMobile);
    ...
    <div className="max-w-7xl mx-auto flex flex-col min-h-[calc(100vh-140px)] animate-fade-up py-2 px-2 sm:px-6">
      ...
      <div className="flex flex-col items-center text-center mb-6 sm:mb-10 mt-2 sm:mt-4 px-2 sm:px-4">
        <div className="inline-flex items-center gap-3 px-3 py-1 bg-primary/5 rounded-full border border-primary/20 mb-3 sm:mb-4">
          ...
          <span className="text-[9px] sm:text-[10px] font-display text-primary tracking-[0.3em] sm:tracking-[0.4em] uppercase">{metadata.achievement_tag}</span>
        </div>
        ...
        <div className="flex items-center justify-center gap-6 text-[10px] sm:text-xs font-display tracking-[0.15em] sm:tracking-[0.2em] text-muted-foreground uppercase opacity-80 px-4 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-10">
          {successStories.map((story, i) => {
            ...
            return (
              <div 
                key={i} 
                className={`achievement-card group relative bg-white/[0.02] border rounded-xl sm:rounded-2xl overflow-hidden transition-all duration-700 shadow-2xl ${
                  isActive
                    ? "border-primary/40 shadow-primary/5"
                    : "border-white/5 hover:border-primary/40 hover:shadow-primary/5"
                }`}
              >
                <div 
                  className={`absolute top-4 right-6 text-[30px] sm:text-[40px] font-display font-black transition-colors z-0 ${
                    isActive ? "text-primary/10" : "text-white/5 group-hover:text-primary/10"
                  }`}
                >
                  ...
                  <img 
                    ...
                    className={`w-full h-full object-cover transition-all duration-1000 ${
                      isActive
                        ...
                        : "opacity-40 group-hover:opacity-100 group-hover:scale-110 grayscale group-hover:grayscale-0"
                    }`} 
                  />
                ...
                <div className="p-5 sm:p-8 relative z-10">
                  <h3 
                    className={`text-lg sm:text-xl md:text-2xl font-display font-bold tracking-tight leading-tight transition-colors text-glow-hover ${
                  ...
                  <p 
                    className={`text-xs sm:text-sm leading-relaxed font-light transition-colors duration-500 mt-3 sm:mt-4 ${
                  ...
                  {story.link && (
                    <div className="pt-4 sm:pt-6">
                      <a 
                        href={story.link} 
                        target="_blank" 
                        className={`inline-flex items-center gap-2 text-[10px] font-display font-bold tracking-[0.2em] transition-all group/link ${
                      ...
                      >
                        ...
                        <svg 
                          ...
                          className={`transition-transform ${
                            isActive ? "translate-x-1" : "group-hover/link:translate-x-1"
                          }`}
                        >
                ...
                <div 
                  className={`absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-primary/20 to-transparent transition-transform duration-700 ${
                    isActive ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"
                  }`} 
                />
                <div 
                  className={`absolute top-0 left-0 w-[1px] h-full bg-gradient-to-b from-transparent via-primary/20 to-transparent transition-transform duration-700 ${
