      ...
      <div className="flex-1 overflow-y-auto pb-10 no-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-10">
          {successStories.map((story, i) => (
            <div key={i} className="group relative bg-white/[0.02] border border-white/5 rounded-xl sm:rounded-2xl overflow-hidden hover:border-primary/40 transition-all duration-700 shadow-2xl hover:shadow-primary/5">
              <div className="absolute top-4 right-6 text-[30px] sm:text-[40px] font-display font-black text-white/5 group-hover:text-primary/10 transition-colors z-0">
              ...
              <div className="p-5 sm:p-8 relative z-10">
                <h3 className="text-lg sm:text-xl md:text-2xl font-display font-bold tracking-tight leading-tight group-hover:text-primary transition-colors text-glow-hover text-white">{story.title}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground/60 leading-relaxed font-light group-hover:text-foreground transition-colors duration-500 mt-3 sm:mt-4">{story.description}</p>
                {story.link && (
