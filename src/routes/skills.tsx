...
function CategoryRadarModal({ category, onClose }: { category: SkillCategory; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 md:p-10 animate-in fade-in duration-500">
      <div className="absolute inset-0 bg-black/95 backdrop-blur-2xl" onClick={onClose} />

      <div className="relative w-full max-w-4xl bg-[#080808] border border-primary/30 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-10 shadow-[0_0_100px_rgba(var(--primary),0.15)] overflow-hidden animate-in zoom-in-95 duration-500 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start mb-5 sm:mb-8 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[8px] font-mono text-primary tracking-[0.3em] sm:tracking-[0.4em] uppercase">Tactical_Intelligence_Overlay</span>
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-display font-bold tracking-tighter text-white uppercase">
              {category.title}<span className="text-primary">_</span>
            </h2>
          </div>
          <button onClick={onClose} className="p-2 bg-white/5 border border-white/10 rounded-full hover:bg-primary/20 hover:border-primary transition-all duration-300 shrink-0">
            <X className="size-4 sm:size-5 text-white/60" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center relative z-10">
          ...
          <div className="flex justify-center">
            <svg width={size} height={size} className="overflow-visible drop-shadow-[0_0_20px_rgba(var(--primary),0.1)] max-w-full">
              ...
              {axes.map((axis, i) => {
                ...
                const textX = center + (radius + 26) * Math.cos(i * angleStep - Math.PI / 2);
                const textY = center + (radius + 26) * Math.sin(i * angleStep - Math.PI / 2);
                return (
                  <g key={axis}>
                    <line x1={center} y1={center} x2={x} y2={y} className="stroke-white/10 stroke-[0.5px]" />
                    <text x={textX} y={textY} textAnchor="middle" className="fill-white/40 text-[6.5px] font-mono tracking-widest uppercase">{axis}</text>
                  </g>
                );
              })}
              <polygon
                points={category.skills.map((skill, i) => {
          ...
          <div className="grid grid-cols-2 gap-3">
            {category.skills.map((skill, i) => (
              <div
                ...
                onTouchStart={() => setHighlightedSkill(skill.name)}
                onTouchEnd={() => setHighlightedSkill(null)}
                className={`p-3 sm:p-4 bg-white/[0.03] border rounded-xl transition-all duration-300 cursor-crosshair ${
                  highlightedSkill === skill.name
                    ? "border-primary/50 bg-primary/5 shadow-[0_0_20px_rgba(var(--primary),0.1)] scale-[1.02]"
                    : "border-white/5"
                }`}
              >
                <div className="flex items-center gap-2 mb-2">
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto flex flex-col min-h-[calc(100vh-140px)] animate-fade-up py-2 px-2 sm:px-6">
        ...
        {/* Header */}
        <div className="flex flex-col items-center text-center mb-6 sm:mb-10 mt-2 sm:mt-4 px-2 sm:px-4">
          <div className="inline-flex items-center gap-3 px-3 py-1 bg-primary/5 rounded-full border border-primary/20 mb-3 sm:mb-4">
            ...
            <span className="text-[9px] sm:text-[10px] font-display text-primary tracking-[0.3em] sm:tracking-[0.4em] uppercase">{metadata.intelligence_tag}</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pb-10 no-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 sm:gap-12 gap-y-10 sm:gap-y-16">
            {skills.map((category, catIdx) => (
              <div key={category.title} className="flex flex-col group">
                <div className="flex items-center justify-between border-b border-white/5 pb-3 mb-4 sm:mb-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] font-mono text-primary/30 font-bold">0{catIdx + 1}</span>
                    <h3 className="text-[10px] sm:text-[11px] font-display font-bold tracking-[0.3em] text-white/70 group-hover:text-primary transition-colors duration-500 uppercase">
                      {category.title}
                    </h3>
                  </div>
                  <button
                    onClick={() => setSelectedCategory(category)}
                    className="flex items-center gap-2 px-2.5 py-1 bg-white/5 border border-white/10 rounded-full hover:bg-primary/20 hover:border-primary transition-all duration-300 opacity-0 group-hover:opacity-100"
                  >
                ...
                <div className="space-y-5 sm:space-y-6 cursor-pointer" onClick={() => setSelectedCategory(category)}>
                  {category.skills.map((s) => (
                    <div key={s.name} className="group/skill">
                      <div className="flex justify-between items-end mb-2">
                        <p className="text-[10px] sm:text-[11px] font-display font-bold tracking-[0.15em] text-white/30 group-hover/skill:text-white transition-all duration-300 uppercase">
                          {s.name}
                        </p>
