    ...
    <div className="max-w-7xl mx-auto flex flex-col min-h-[calc(100vh-140px)] animate-fade-up py-2 px-2 sm:px-6">
      ...
      <div className="flex flex-col items-center text-center mb-6 sm:mb-10 mt-2 sm:mt-4 px-2 sm:px-4">
        <div className="inline-flex items-center gap-3 px-3 py-1 bg-primary/5 rounded-full border border-primary/20 mb-3 sm:mb-4">
          ...
          <span className="text-[9px] sm:text-[10px] font-display text-primary tracking-[0.3em] sm:tracking-[0.4em] uppercase">{metadata.operational_log_tag}</span>
        </div>
      ...
      {/* Timeline */}
      <div className="flex-1 overflow-y-auto pb-20 no-scrollbar relative px-2" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <style dangerouslySetInnerHTML={{ __html: '.no-scrollbar::-webkit-scrollbar { display: none; }' }} />

        {/* Timeline spine — left on mobile, centered on desktop */}
        <div className="absolute left-4 sm:left-8 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-primary via-primary/20 to-transparent md:transform md:-translate-x-1/2 z-0" />

        <div className="space-y-12 sm:space-y-20 md:space-y-32 relative z-10">
          {experience.map((ex, i) => {
            const isLeft = i % 2 === 0;
            ...
            return (
              ...
              >
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="text-[9px] font-mono text-primary tracking-[0.3em] uppercase">{ex.ref || "ARCHIVE_REF // NULL"}</span>
                        {ex.duration && (
                ...
                <div className={`hidden md:block w-full md:w-[42%] ${isLeft ? "md:order-1 md:text-right" : "md:order-3 md:text-left"}`}>
                ...
                <div className={`hidden md:block w-full md:w-[42%] mt-6 md:mt-0 ${isLeft ? "md:order-3 md:text-left" : "md:order-1 md:text-right"}`}>
                  <div className={`space-y-4 transition-all duration-700 ${isLeft ? "md:translate-x-[15px] group-hover:translate-x-0" : "md:translate-x-[-15px] group-hover:translate-x-0"}`}>
                    <div className={`flex flex-col ${isLeft ? "md:items-start" : "md:items-end"} gap-1.5`}>
                      ...
                      <h3 className="text-xl md:text-2xl font-display font-bold tracking-tight leading-none group-hover:text-primary transition-all text-white">{ex.title}</h3>
                    </div>
                    <p className={`text-[11px] text-muted-foreground/60 leading-relaxed font-light max-w-sm ${isLeft ? "" : "md:ml-auto"} group-hover:text-foreground transition-colors duration-500 line-clamp-3`}>{ex.description}</p>
